...
DEFUN(strtod, (nptr, endptr), CONST char *nptr AND char **endptr)
{
  ...
  /* FIXME: hack to avoid generationg the fzero instruction, not implemented by RSIM */
  int izero = 0;
  double zero = (double) izero;
  ...
  got_dot = 0;
  num = zero; // FIXME: hack to avoid fzero
  got_digit = 0;
  exponent = 0;
 ...
 underflow:
  ...
  if (endptr != NULL)
    *endptr = (char *) nptr;
  errno = ERANGE;
  return zero; // FIXME: hack to avoid fzero
